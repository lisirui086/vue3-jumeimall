  ...
  <div class="order-item">
    ...
    <div class="body">
      <div class="column goods">
        <ul>
          <li v-for="goods in order.skus" :key="goods.id">
            <router-link class="image" :to="`/product/${goods.skuId}`">
      ...
      <div class="column action">
        ...
        <!-- 已完成：查看详情，再次购买，申请售后 -->
        <!-- 已取消：查看详情 -->
        <XtxButton type="primary" size="small" v-if="order.orderState === 1" @click="$router.push(`/member/pay?orderId=${order.id}`)">立即付款</XtxButton>
        ...
        <p><a href="javascript:;" v-if="order.orderState === 1" @click="$emit('on-cancel', order)">取消订单</a></p>
        <p><a href="javascript:;" v-if="[2,3,4,5].includes(order.orderState)">再次购买</a></p>
        <p><a href="javascript:;" v-if="[4,5].includes(order.orderState)">申请售后</a></p>
      </div>
    </div>
  </div>
  <div class="order-item">
